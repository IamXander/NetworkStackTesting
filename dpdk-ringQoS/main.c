...
/**
 * @brief Every core is a generator. Each generator has some params.
 * 
 */
...
/**
 * @brief Generator core. One thread per cord per function call.
 * 
 * @param gc (generatorConfig_t*)
 */
static void lcore_main_generate_packets(void * gc) {
	...
	while (packetsSent < generatorConfig->packetsToGenerate) {
		...
		for (int i = 0; i < generatorConfig->burstSize; i++) {
			...
			// decide where the packets belong (e.g. which queue, subport etc...)
		...
		uint16_t nb_tx = 0;
		do {
			// Put the packets on a ring that will be popped off by the main core
...
/**
 * @brief Main core doing QoS work. According to DPDK only one core should do QoS or performace will take a large hit.
 * 
 * @param generatorConfigs configs of all the other generators
 * @param numGenerators 
 * @param numQoSDequeue per dequeue operator from the QoS framework how many packets should be popped off
 * @param sleeptime if != 0 then call nanosleep with that amount of nanos seconds. nanosleep will take around >1us just to complete the call so passing 1ns will round up to >1us.
 */
static void lcore_main_QOS(struct generatorConfig_t* generatorConfigs, int64_t numGenerators, int64_t numQoSDequeue, uint64_t sleeptime) {
	...
	while (morePackets) {
		...
		for (int64_t i = 0; i < numGenerators; ++i) {
			...
			uint16_t nb_tx = 0;
			do {
				// Take packets generated by cores of the queue
			...
			if (numQoSDequeue == -1) { //No QoS
				for (int j = 0; j < generatorConfigs[i].packetsToDequeue; ++j) {
					// If not using QoS free the packet
					rte_pktmbuf_free(bufs[j]);
				}
			} else {
				// for (int i = 0; i < generatorConfigs[i].packetsToDequeue; i++) {
				// 	rte_sched_port_pkt_write(scheduler, bufs[i], generatorConfigs[i].subport, generatorConfigs[i].pipe, generatorConfigs[i].traffic_class, generatorConfigs[i].queue, e_RTE_METER_GREEN);
				// }
				int numEnqueue = 0;
				// do {
				// Enqueue or drop the packet(s)
		...
		if (numQoSDequeue != -1) {
			...
			// Dequeue packets. If there was not enough time dequeue will not return the full amount of packets.
	...
	// Wait for other cores to complete for python output
	int lcore_id;
	RTE_LCORE_FOREACH_SLAVE(lcore_id) {
		rte_eal_wait_lcore(lcore_id);
	}

	printf("@@@DATA@@@\n"); //Marker for python! Easy way to mine data for python
	printf("{\n");
	for (int i = 0; i < numGenerators; i++) {
		printf("\"Core%d\": {\"dropped\": %ld, \"survived\": %ld, \"total\": %ld},\n", i, packetsDropped[i], generatorConfigs[i].packetsToGenerate - packetsDropped[i], generatorConfigs[i].packetsToGenerate);
	}
	printf("\"packets_dequeued\": %ld,\n", totalDeq);
	printf("\"packets_dropped\": %ld,\n", totalPacketsDropped);
	printf("\"packets_left\": %ld,\n", totalPackets - (totalDeq + totalPacketsDropped));
	printf("\"duration\": %f\n}", seconds);
}
